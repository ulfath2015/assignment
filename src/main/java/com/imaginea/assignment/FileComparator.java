	...
	public FileComparator(String propFile) {
		...
		try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream(propFile)) {
			...
			if (inputStream != null) {
				...
				sourceNames = new HashSet<String>(Files.readAllLines(new File(props.get("SOURCE1").toString()).toPath(),Charset.forName("UTF-8")));
				targetNames = new HashSet<String>(Files.readAllLines(new File(props.get("SOURCE2").toString()).toPath(),Charset.forName("UTF-8")));
			} 
			else {
